                ...
                <div class="flex flex-col md:flex-row justify-center items-center">
                    ...
                    @auth
                        @if(auth()->user()->username != request()->route('username'))
                            <form action="{{ $user->followers->contains('id', auth()->id()) ? route('unfollow', $user->username) : route('follow', $user->username) }}" method="post">
                                @csrf
                                <button type="submit" class="px-4 py-2 bg-primary text-white rounded-lg hover:opacity-70 transition duration-300 ease">{{ $user->followers->contains('id', auth()->id()) ? 'Unfollow' : 'Follow' }}</button>
                            </form>
                        @else
